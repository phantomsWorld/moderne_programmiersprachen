...
class Field ( player: Int, height: Int, width: Int ) {
  ...
  def randomColor:Int = (Math.random*5).toInt
  ...
  def playerStart(player:Int):Cell = if(player == 0) cells(height-1)(0) else cells(0)(width-1)
  ...
  override def toString = {
    ...
    for(i <-0 until height) {
      for(j <-0 until width) result += cells(i)(j).toString + " "
