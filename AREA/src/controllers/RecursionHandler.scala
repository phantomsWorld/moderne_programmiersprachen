...
class RecursionHandler(fieldController: Fieldcontroller, recDepth: Int, botID: Int) {
  ...
  def updateRecursionNeighbors {
    for (recNum <- 0 until recDepth; actualColor <- 0 until (math.pow(fieldController.colorNum, (recNum + 1))).toInt) {
      if (recNum == 0) {
        recursionNeighbors(actualColor).list = fieldController.recursiveBotNeighbors(fieldController.ownedCells(botID).list, actualColor)
      } else {
        recursionNeighbors(recStartArray(recNum) + actualColor).list =
          fieldController.recursiveBotNeighbors(recursionNeighbors(recStartArray(recNum - 1) + actualColor / fieldController.colorNum).list, actualColor % fieldController.colorNum)
      }
    }
  }
}
