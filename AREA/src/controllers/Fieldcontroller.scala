...
class Fieldcontroller(playerCount: Int, height: Int, width: Int, useBot: Boolean) {
  ...
  def updateOwnedNeighbors(player: Int, cell: Cell): Boolean = {
    ...
    for (i <- 0 until 4) {
      ...
      if (!ownedCells(player).list.contains(neighborCell) && !ownedCells((player + 1) % 2).list.contains(neighborCell)
          && (cell == neighborCell == false) && (cell.compareColors(neighborCell))) {
  ...
  def recursiveBotNeighbors(visitedCells: List[Cell], nextCheckingColor: Int): List[Cell] = {
    ...
    if (neighborFound) list = recursiveBotNeighbors(list, nextCheckingColor)
  ...
  def changeColorWithBotInformation(newColor: Int, callBot: Boolean) = {
    if (newColor == field.playerStart(0).c || newColor == field.playerStart(1).c) {
      println("Incorrect color selected!!!\nColors: " + newColor + "; Player: " + playerOnTurn)
    }
    else {
  ...
  def calculatedPossession = Array(100 * ownedCells(0).list.length.toDouble / (field.cells.length * field.cells(0).length),
                                   100 * ownedCells(1).list.length.toDouble / (field.cells.length * field.cells(0).length))
  ...
  def parseXML(data: xml.Elem) = {
    ...
    for (cell <- data \\ "cell") field.cells((cell \\ "@xValue").text.toInt)((cell \\ "@yValue").text.toInt) = 
      new Cell((cell \\ "@xValue").text.toInt, (cell \\ "@yValue").text.toInt, cell.text.toInt)
  ...
  def saveXML(filename: String) = {
    var myXML: scala.xml.Node = <field><gameInfos><player>{ playerCount }</player><height>{ field.h.toString }</height>
                                <width>{ field.w.toString }</width><playerOnTurn>{ playerOnTurn }</playerOnTurn></gameInfos>{ field.toXML }</field>
  ...
  def fieldToHtmlString = {
    ...
    for (i <- 0 until height; j <- 0 until width) result = 
      result + (if (j == 0) "<tr>" else "") + "<td id='" + i + "-" + j + "' class='" + field.cells(i)(j).toString + 
      "'><a href='#' class='changeColorField'><img src='" + field.cells(i)(j).toString + ".png' alt='" + field.cells(i)(j).toString +
      "' class='" + (if (ownedCells(0).list.contains(field.cells(i)(j)) == true) "glow0"
        else if (ownedCells(1).list.contains(field.cells(i)(j)) == true) "glow1" else "") + "' /></a></td>" + (if (j == width - 1) "</tr>" else "")
